Feature: chat/reference-id
  ...
  Scenario: too long references dont break the api
    Given user "participant1" creates room "group room"
      | roomType | 2 |
      | roomName | room |
    When user "participant1" sends message "Message 2" with reference id "1234567890123456789012345678901234567890" to room "group room" with 201
    When user "participant1" sends message "Message 1" with reference id "too long ref is cut off 123456789012345678901234567890" to room "group room" with 201
    Then user "participant1" sees the following messages in room "group room" with 200
      | room       | actorType | actorId      | actorDisplayName         | message   | messageParameters | referenceId |
